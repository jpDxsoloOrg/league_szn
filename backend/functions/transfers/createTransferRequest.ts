...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    const division = await getRepositories().divisions.findById(toDivisionId);
    ...
    const pendingRequests = await getRepositories().transfers.listPendingByPlayer(playerId);

    if (pendingRequests.length > 0) {
      return badRequest('You already have a pending transfer request. Cancel it before submitting a new one.');
    }

    const item = await getRepositories().transfers.create({
      playerId,
      fromDivisionId,
      toDivisionId,
      reason: reason.trim(),
