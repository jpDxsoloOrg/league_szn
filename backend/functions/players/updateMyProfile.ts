...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    if (setExpressions.length === 1) {
      return badRequest('No valid fields to update. Please provide a valid field from the following list: ' + ALLOWED_FIELDS.join(', ') + '. Example usage: { "name": "John Doe", "bio": "Short bio here." }');
    }
...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  if (field === 'bio' && value.trim().length === 0) {
    return badRequest('Bio cannot be empty or whitespace-only');
  }
  if (field === 'name' && value.trim().length === 0) {
    return badRequest('Name cannot be empty');
  }
