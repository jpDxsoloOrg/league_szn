...
describe('getPlayers', () => {
  ...
  it('filters out players without currentWrestler', async () => {
    ...
    const store = (repos.roster.players as unknown as { store: Map<string, Record<string, unknown>> }).store;
...
describe('deletePlayer', () => {
  ...
  it('cleans up season standings on delete', async () => {
    ...
    await repos.season.seasonStandings.increment('s1', player.playerId, { wins: 1 });
    await repos.season.seasonStandings.increment('s2', player.playerId, { wins: 2 });
    ...
    const remaining = await repos.season.seasonStandings.listByPlayer(player.playerId);
...
describe('getMyProfile', () => {
  ...
  it('returns player profile with season records for Wrestler', async () => {
    ...
    await repos.season.seasonStandings.increment(season.seasonId, player.playerId, { wins: 5 });
    await repos.season.seasonStandings.increment(season.seasonId, player.playerId, { losses: 2 });
    await repos.season.seasonStandings.increment(season.seasonId, player.playerId, { draws: 1 });
