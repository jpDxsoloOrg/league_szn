...
interface BulkCreateStipulationsBody {
  names: string[];
}
...
export const handler: APIGatewayProxyHandler = async (event) => {
  try {
    const { data: body, error: parseError } = parseBody<BulkCreateStipulationsBody>(event);
    ...
    if (body.names.length > MAX_BULK_CREATE) {
      return badRequest(`Maximum ${MAX_BULK_CREATE} stipulations can be created at once`);
    }
    ...
    for (const name of uniqueNames) {
      ...
      createdItems.push(stipulation);
    }

    return created({ created: createdItems.length, stipulations: createdItems });
  } catch (err) {
    console.error('Error bulk creating stipulations:', err);
    return serverError('Failed to bulk create stipulations');
  }
};
