  ...
  useTranslation: () => ({
    t: (key: string) => {
      const map: Record<string, string> = {
        ...
        'auth.wrestlerAssignedLater':
          'You don\'t pick a wrestler here — an admin sets you up with one.',
...
describe('Signup', () => {
  ...
  it('does not ask for a wrestler', () => {
    renderSignup();

    expect(screen.queryByLabelText('Wrestler Name')).not.toBeInTheDocument();
    expect(screen.queryByRole('combobox')).not.toBeInTheDocument();
    expect(
      screen.getByText(/an admin sets you up with one/i)
    ).toBeInTheDocument();
  });
  ...
  it('calls signUp with the player details and no wrestler', async () => {
    const user = userEvent.setup();
    ...
    await waitFor(() => {
      expect(mockSignUp).toHaveBeenCalledWith(
        'wrestler@league.com',
        'StrongPw1',
        { playerName: 'DiscordGuy', psnId: 'PSN_Player1' }
      );
    });
